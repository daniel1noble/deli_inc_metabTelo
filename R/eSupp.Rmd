...
#Read in model
mod.imp.1.obj <- readRDS("~/Dropbox/1 - PhD/5 - Ldeli_inc_metabTelo/output/rds/mod.imp.1")
...
#Combining the coefficients from the final model
#Getting the fixed effects
mod.imp.1_coeffs <- mod.imp.1_post[1:15] %>% posterior_summary() %>% round(3) 
#Excluding est. error
...
#Posterior mode
cold_rpt_mode <- bind_rows(lapply(temp, function(x) brms_rpt(x, model = cc_cold.2.obj, type = "mode")))
cold_rpt_mode %<>%  mutate(treatment = "cold")

hot_rpt_mode <- bind_rows(lapply(temp, function(x) brms_rpt(x, model = cc_hot.2.obj, type = "mode")))
...
#Combining the coefficients from the final model
#Getting the fixed effects
cold_imp_coeffs <- cold_imp.post[1:12] %>% posterior_summary() %>% round(3) 
#Excluding est. error
...
#Combining the coefficients from the final model
#Getting the fixed effects
hot_imp_coeffs <- hot_imp.post[1:12] %>% posterior_summary() %>% round(3) 
#Excluding est. error
...
#Extract entire posterior
cc_cold_imp.post <- posterior_samples(cc_cold.2.obj)
#Names of each column
colnames(cc_cold_imp.post)[1:10]

#Combining the coefficients from the final model
#Getting the fixed effects
cc_cold_imp_coeffs <- cc_cold_imp.post[1:10] %>% posterior_summary() %>% round(3) 
#Excluding est. error
cc_cold_imp_coeffs <- cc_cold_imp_coeffs[,c(1,3,4)]
#Rownames as col
cc_cold_imp_coeffs <- as.data.frame(cc_cold_imp_coeffs)
cc_cold_imp_coeffs$Parameter <- c(rownames(cc_cold_imp_coeffs))
rownames(cc_cold_imp_coeffs) <- NULL 
cc_cold_imp_coeffs <- cc_cold_imp_coeffs[,c(4,1:3)]

#Extract entire posterior
cc_hot_imp.post <- posterior_samples(cc_hot.2.obj)
#Names of each column
colnames(cc_hot_imp.post)[1:10]

#Combining the coefficients from the final model
#Getting the fixed effects
cc_hot_imp_coeffs <- cc_hot_imp.post[1:10] %>% posterior_summary() %>% round(3) 
#Excluding est. error
cc_hot_imp_coeffs <- cc_hot_imp_coeffs[,c(1,3,4)]
#Rownames as col
cc_hot_imp_coeffs <- as.data.frame(cc_hot_imp_coeffs)
cc_hot_imp_coeffs$Parameter <- c(rownames(cc_hot_imp_coeffs))
rownames(cc_hot_imp_coeffs) <- NULL 
cc_hot_imp_coeffs <- cc_hot_imp_coeffs[,c(4,1:3)]
```

```{r, eval = TRUE, include = TRUE}
pander(cc_cold_imp_coeffs,
       justify = "lccc",
       #split.cell = 80,
...
# Read in models
cold.mod.imp.4 <- readRDS("~/Dropbox/1 - PhD/5 - Ldeli_inc_metabTelo/output/rds/cold.mod.imp.4")
hot.mod.imp.4 <- readRDS("~/Dropbox/1 - PhD/5 - Ldeli_inc_metabTelo/output/rds/hot.mod.imp.4")
...
#Combining the coefficients from the final model
#Getting the fixed effects
cold_imp.4_coeffs <- cold_imp.4.post[1:13] %>% posterior_summary() %>% round(3) 
#Excluding est. error
...
#Combining the coefficients from the final model
#Getting the fixed effects
hot_imp.4_coeffs <- hot_imp.4.post[1:12] %>% posterior_summary() %>% round(3) 
#Excluding est. error
...
#CC
cc_cold.4.obj <- readRDS("~/Dropbox/1 - PhD/5 - Ldeli_inc_metabTelo/output/rds/cc_cold.4.obj")
cc_hot.4.obj <- readRDS("~/Dropbox/1 - PhD/5 - Ldeli_inc_metabTelo/output/rds/cc_hot.4.obj")
...
#Extract entire posterior
cc_cold_imp.4.post <- posterior_samples(cc_cold.4.obj)
#Names of each column
colnames(cc_cold_imp.4.post)[1:13]

#Combining the coefficients from the final model
#Getting the fixed effects
cc_cold_imp.4_coeffs <- cc_cold_imp.4.post[1:13] %>% posterior_summary() %>% round(3) 
#Excluding est. error
cc_cold_imp.4_coeffs <- cc_cold_imp.4_coeffs[,c(1,3,4)]
#Rownames as col
cc_cold_imp.4_coeffs <- as.data.frame(cc_cold_imp.4_coeffs)
cc_cold_imp.4_coeffs$Parameter <- c(rownames(cc_cold_imp.4_coeffs))
rownames(cc_cold_imp.4_coeffs) <- NULL 
cc_cold_imp.4_coeffs <- cc_cold_imp.4_coeffs[,c(4,1:3)]

#Extract entire posterior
cc_hot_imp.4.post <- posterior_samples(cc_hot.4.obj)
#Names of each column
colnames(cc_hot_imp.4.post)[1:13]

#Combining the coefficients from the final model
#Getting the fixed effects
cc_hot_imp.4_coeffs <- cc_hot_imp.4.post[1:12] %>% posterior_summary() %>% round(3) 
#Excluding est. error
cc_hot_imp.4_coeffs <- cc_hot_imp.4_coeffs[,c(1,3,4)]
#Rownames as col
cc_hot_imp.4_coeffs <- as.data.frame(cc_hot_imp.4_coeffs)
cc_hot_imp.4_coeffs$Parameter <- c(rownames(cc_hot_imp.4_coeffs))
rownames(cc_hot_imp.4_coeffs) <- NULL 
cc_hot_imp.4_coeffs <- cc_hot_imp.4_coeffs[,c(4,1:3)]
```

```{r, eval = TRUE, include = TRUE}
pander(cc_cold_imp.4_coeffs,
       justify = "lccc",
       #split.cell = 80,
